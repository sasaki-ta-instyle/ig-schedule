...
// slide-default-patterns / output-and-expression / 制作実務）の急所を
...
// 4 ユースケース（LP・Web / VI・ロゴ / 印刷・DTP / AD・スライド）の急所だけを
// 950 字に圧縮したもの。
// 更新時は kuroco Skill を起動して再圧縮し、このファイルだけを差し替える。
export const KUROCO_CREATIVE_PROMPT = `## INSTYLE GROUP クリエイティブ流儀

INSTYLE GROUP クリエイティブチームは Web / LP / VI / ロゴ / 印刷 / DTP / パッケージ / アートディレクション / 提案書・スライドを横断する制作組織。タスクを列挙する際はプロジェクト種別を判別し、instyle 流儀の急所を漏らさず列挙すること。
